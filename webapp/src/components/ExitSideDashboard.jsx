 ...
 */
export default function ExitSideDashboard({ results }) {
                            ...
                            onClick={async () => {
                                try {
                                    ...
                                    if (response.ok) {
                                        ...
                                        a.href = url;
                                        a.download = `Exit_Side_Report_${results.case_id || 'DEMO'}.pdf`;
                                        a.click();
                ...
                <div className="panel" style={{ padding: '1.25rem' }}>
                    ...
                    {probableGuards.length > 0 ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                            {probableGuards.slice(0, 5).map((guard, idx) => (
                                <div
                                    key={idx}
                                    style={{
                                        padding: '0.75rem',
                                ...
                                >
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                        ...
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            ...
                                            <p style={{ fontSize: '0.625rem', color: '#9ca3af', margin: 0 }}>
                                                {guard.country} • {guard.isp?.substring(0, 25)}
                                            </p>
                                        </div>
                                        <div style={{ textAlign: 'right' }}>
                                            <p style={{
                                                ...
                                                {(guard.guard_probability * 100).toFixed(0)}%
                                            </p>
                                            {guard.in_consensus && (
                                                <span style={{
                                                    fontSize: '0.5rem',
                                    ...
                                    {guard.reason && (
                                        <p style={{ fontSize: '0.5625rem', color: '#f59e0b', marginTop: '0.5rem' }}>
                                            🔬 {guard.reason}
                                        </p>
                                    )}
                                </div>
                        ...
                        <div style={{ textAlign: 'center', padding: '2rem', color: '#6b7280' }}>
                            <AlertTriangle style={{ width: '2rem', height: '2rem', margin: '0 auto 0.5rem', opacity: 0.5 }} />
                            <p style={{ fontSize: '0.75rem' }}>No guard nodes predicted</p>
                            <p style={{ fontSize: '0.625rem', color: '#4b5563' }}>Insufficient traffic patterns for inference</p>
                        </div>
                    )}
                </div>

                {/* CENTER: Detected Exit Nodes */}
                    ...
                    <div className="panel" style={{ padding: '1.25rem' }}>
                        ...
                        {topExits.length > 0 ? (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                {topExits.slice(0, 5).map((exit, idx) => (
                                    <div key={idx} style={{
                                        display: 'flex',
                                        alignItems: 'center',
                                        gap: '0.5rem',
                                        ...
                                        <div style={{ textAlign: 'right' }}>
                                            ...
                                            {exit.in_consensus && (
                                                <CheckCircle style={{ width: '0.75rem', height: '0.75rem', color: '#10b981' }} />
                                            )}
                                        </div>
                                    </div>
                                ))}
                    ...
                    <div className="panel" style={{ padding: '1.25rem' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.75rem' }}>
                            <div style={{ backgroundColor: 'rgba(255,255,255,0.03)', padding: '0.5rem', borderRadius: '0.25rem' }}>
                                <p style={{ fontSize: '0.625rem', color: '#9ca3af', margin: 0 }}>Burst Entropy</p>
                                <p style={{ fontFamily: 'monospace', fontSize: '0.875rem', color: 'white', margin: 0 }}>
                                    {fingerprint.burst_entropy?.toFixed(3) || 'N/A'}
                                </p>
                            </div>
                            <div style={{ backgroundColor: 'rgba(255,255,255,0.03)', padding: '0.5rem', borderRadius: '0.25rem' }}>
                                <p style={{ fontSize: '0.625rem', color: '#9ca3af', margin: 0 }}>Micro-gap Avg</p>
                                <p style={{ fontFamily: 'monospace', fontSize: '0.875rem', color: 'white', margin: 0 }}>
                                    {fingerprint.micro_gap_avg ? `${(fingerprint.micro_gap_avg * 1000).toFixed(2)}ms` : 'N/A'}
                                </p>
                            </div>
                            <div style={{ backgroundColor: 'rgba(255,255,255,0.03)', padding: '0.5rem', borderRadius: '0.25rem' }}>
                                <p style={{ fontSize: '0.625rem', color: '#9ca3af', margin: 0 }}>Size Var Slope</p>
                                <p style={{ fontFamily: 'monospace', fontSize: '0.875rem', color: 'white', margin: 0 }}>
                                    {fingerprint.size_variance_slope?.toFixed(2) || 'N/A'}
                                </p>
                            </div>
                            <div style={{ backgroundColor: 'rgba(255,255,255,0.03)', padding: '0.5rem', borderRadius: '0.25rem' }}>
                                <p style={{ fontSize: '0.625rem', color: '#9ca3af', margin: 0 }}>Circuit Lifetime</p>
                                <p style={{ fontFamily: 'monospace', fontSize: '0.875rem', color: 'white', margin: 0 }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                ...
                                <span style={{ fontFamily: 'monospace', color: '#f59e0b' }}>
                                    {flowMetadata.cdn_filtered || 0}
                                </span>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <span style={{ fontSize: '0.75rem', color: '#9ca3af' }}>Unique Exit IPs</span>
